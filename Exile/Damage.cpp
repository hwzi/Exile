	...
	namespace SDK
	{
		...
		float Damage::CalculatePhysicalDamage(GameObject* source, GameObject* target, float amount) {
			...
			if (sourceType == GameObjectType::AIHeroClient && target->Flags() & GameObjectFlags_AIHeroClient) {
				if (source->FindBuff(0xD4515DC0)) { // "ASSETS/Perks/Styles/Precision/Conqueror/ConquerorEnrage.lua"
		...
		float Damage::CalculateMagicalDamage(GameObject * source, GameObject * target, float amount) {
			...
			if (target->FindBuff(0x8C10C58A)) { // "cursedtouch"
...
#pragma region Target buffs
			if (targetFlags & GameObjectFlags_AIHeroClient) {
				...
				case Character::Annie: {
					if (target->FindBuff(0x668C0F2F)) { // "MoltenShield"
						auto spell = target->GetSpell(SpellSlot_E);
						result.Percent *= 1.0f - spell->SpellData->Resource->eEffect1[spell->Level] / 100.0f;
					}
					break;
				}
				case Character::Braum: {
					if (target->FindBuff(0x7215EE81)) { // "braumeshieldbuff"
						auto spell = target->GetSpell(SpellSlot_E);
				...
				case Character::Galio: {
					if (damageType == DamageType_Physical && target->FindBuff(0x22086614)) { // "GalioW"
						auto spell = target->GetSpell(SpellSlot_W);
				...
				case Character::Malzahar: {
					if (target->FindBuff(0x33DB7C71)) { // "malzaharpassiveshield"
						result.Percent *= 0.1f;
					}
					break;
				}
				case Character::MasterYi: {
					if (target->FindBuff(0x50E753F4)) { // "Meditate"
						auto spell = target->GetSpell(SpellSlot_W);
			...
			if (sourceFlags & GameObjectFlags_AIHeroClient) {
				for (auto buffManager : source->Buffs) {
					...
					case 0xAF037A39: // "sonapassivedebuff"
						result.Percent *= 1 - (0.25f + 0.0004f * caster->TotalAbilityPower());
						break;
...
#pragma region Source items
			...
			for (auto i = 0; i < 6; i++) {
				...
				case Item::Titanic_Hydra:
					...
					result.PhysicalDamage += source->FindBuff(0x9848DC51) ? 40.0f + 0.1f * source->MaxHealth : 5.0f + 0.01f * source->MaxHealth;
					break;
				case Item::Dead_Mans_Plate: {
					auto buff = source->FindBuff(0xBB077487); // "dreadnoughtmomentumbuff"
					if (buff) {
...
#pragma region Source buffs
			for (auto buffManager : source->Buffs) {
				...
				case 0xF791AC5A: { // "sonaqprocattacker"
					result.MagicalDamage += 5.0f + 5.0f * caster->GetSpell(SpellSlot_Q)->Level + 0.3f * caster->TotalAbilityPower();
					break;
				}
				case 0xE503837D: { // "NamiE"
					result.MagicalDamage += 5.0f + 15.0f * caster->GetSpell(SpellSlot_E)->Level + 0.2f * caster->TotalAbilityPower();
					break;
				}
				...
				case 0x3519F8B0: { // "BlessingoftheLizardElder"
					if (!target->FindBuff(0x48208A75))// blessingofthelizardelderslow
					{
				...
				case 0xBB89EA44: { // "vaynetumblebonus"
					auto spell = caster->GetSpell(SpellSlot_Q);
				...
				case 0x4AED0CE1: { // "cardmasterstackparticle"
					auto spell = caster->GetSpell(SpellSlot_E);
				...
				case 0x8B0B9A1C: { // "bluecardpreattack"
					auto spell = caster->GetSpell(SpellSlot_W);
					auto resource = spell->SpellData->Resource;
					auto spellLevel = spell->Level;
