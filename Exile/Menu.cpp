...
namespace Exile {
	namespace SDK {
		...
		void Menu::Initialize() {
			...
			if (file.is_open()) {
				file >> MenuSave;
				auto& savedBasePositionX = MenuSave["PlatformName"]["MenuSettings"]["BasePosition"]["X"];
				auto& savedBasePositionY = MenuSave["PlatformName"]["MenuSettings"]["BasePosition"]["Y"];
				if (!savedBasePositionX.is_null() && !savedBasePositionY.is_null()) {
					BasePosition = Vector2(savedBasePositionX, savedBasePositionY);
				}
			}

			auto menu = Menu::CreateMenu("PlatformName", "PlatformName");
		...
		void Menu::Dispose() {
			...
			MenuSave["PlatformName"]["MenuSettings"]["BasePosition"]["X"] = BasePosition.X;
			MenuSave["PlatformName"]["MenuSettings"]["BasePosition"]["Y"] = BasePosition.Y;
