...
namespace Exile {
	namespace SDK {
		...
		GameObject* TargetSelector::GetTarget(float range, kDamageType damageType, Vector3 source, bool addBoundingRadius) {
			auto sourcePosition = source.IsValid() ? source : ObjectManager::Player->GetPathController()->ServerPosition;

			if (Config::Advanced::AttackSelectedTarget->Value && SelectedTarget && SelectedTarget->IsValidTarget()) {
				if (Config::Advanced::AttackOnlySelectedTarget->Value || sourcePosition.IsInRange(SelectedTarget->GetPathController()->ServerPosition, range + (addBoundingRadius ? SelectedTarget->GetBoundingRadius() : 0.0f))) {
			...
			for (auto hero : *ManagerTemplate::Heroes) {
				if (hero->IsEnemy() && hero->IsValidTarget() && sourcePosition.IsInRange(hero->GetPathController()->ServerPosition, range + (addBoundingRadius ? hero->GetBoundingRadius() : 0.0f))) {
